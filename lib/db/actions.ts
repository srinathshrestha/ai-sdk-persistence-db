...
import { chats, messages, MyDBMessagePart, parts } from "@/lib/db/schema";
...
export const upsertMessage = async ({
  ...
  const p: MyDBMessagePart[] = message.parts.map((p, i) => {
    switch (p.type) {
      case "text":
        return {
          messageId: result.id,
    ...
    if (part) {
      ...
      switch (part.type) {
        ...
        case "step-start":
          ...
          break;
        case "tool-invocation":
          partData = part.toolInvocation as MyUIMessage["parts"][0];
          break;
        default:
